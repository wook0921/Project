...
$(document).ready(function(){
   ...
   $('#writeComment').on('click',function(e){
      var adminContent = $('#adminContent').val();
      
      if(adminContent = null || adminContent ==''){
         alert("내용을 입력해주세요");
         $('#adminContent').focus();
         return false;
      }
...
<h1 style="margin-top:50px; margin-bottom:67px">QNA</h1>
...
<c:forEach begin="0" end="${fn:length(qnaList)}" items="${qnaList}" var="qnaList">
            <c:if test="${qnaList.Q_PRIVATE eq '1' and myMIDX ne qnaList.M_IDX and ADMIN eq NULL}">
             ...
             <c:if test="${qnaList.C_CONTENT eq NULL and ADMIN ne NULL}">
                 <form action="/admin/insertqnacomment.sosu" method="post">
                    <input type="text" name="C_CONTENT" style="width:615px; border:1px solid #e6e6e6;" id="adminContent"> <input type="submit" id="writeComment" value="댓글 작성" class="write-btn">
                    <input type="hidden" value="${qnaList.M_IDX}" name="M_IDX">
                    <input type="hidden" value="${myMIDX}" name="C_WRITER">
   ...
   <script type="text/javascript">
      function fn_search(pageNo){
         ...
         comSubmit.setUrl("<c:url value='/admin/qnalist.sosu' />");
